    ...
    public function createGroupSavingAccount()
    {
        return $this->transaction(function () {
            return $this->tryCatch(function () {
                ...
                // This row records the opening money coming INTO the group pool —
                // it must be a deposit, or ledger engines debit the pool its own
                // opening balance.
                $TransactionData = $this->transactionUorCFields([
                    'reference' => $code,
                    'gsaid' => $details->id,
                    ...
                    'type' => 'deposit',
                    'narration' => $req['narration'] ?? 'Initial deposit',
                ]);
                $this->UpdateOrCreateRecord('transactions', $TransactionData);
            });
        });
